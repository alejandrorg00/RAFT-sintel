...
def validate_sintel(model, iters=32):
    """Perform validation using the standard Sintel training split."""
    model.eval()
    results = {}
...
def validate_sintel_flyvis_split(
    ...
    """Validate on the FlyVis scene-level Sintel validation split.

    For flyvis_hex=True, the dataset still returns normal RGB/LUM Sintel
    tensors. The FlyVis hex preprocessing is applied here on GPU, matching
    ...
    for dstype in ['clean', 'final']:
        ...
        for val_id in range(len(val_dataset)):
            ...
            epe_list.append(
                epe[valid_mask].detach().cpu().view(-1).numpy()
            )
...
def validate_kitti(model, iters=24):
    """Perform validation on the KITTI-2015 training split."""
    model.eval()
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        if args.dataset == 'chairs':
            ...
            raise ValueError(f"Unknown evaluation dataset: {args.dataset}")